...
func Test_applicationUserDelete(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:        []string{"application", "user", "delete"},
			expectedOut: "",
			expectedErr: "Service Principal or application ID is required (use --spid or --appid)\n",
		},
		{
			name:        "UserID or --all is required with spid",
			args:        []string{"application", "user", "delete", "--spid", "12345"},
			expectedOut: "",
			expectedErr: "UserID (use --userid) OR --all required\n",
		},
	}
