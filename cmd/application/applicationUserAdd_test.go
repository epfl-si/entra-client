...
func Test_applicationUserAdd(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:        []string{"application", "user", "add"},
			expectedOut: "",
			expectedErr: "Service Principal or application ID is required (use --spid or --appid)\n",
		},
		{
			name:        "UserID is required with spid",
			args:        []string{"application", "user", "add", "--spid", "12345"},
			expectedOut: "",
			expectedErr: "UserID is required (use --userid)\n",
		},
	}
